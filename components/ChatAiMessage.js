...
import { useMemo, useState, useCallback, Fragment, createElement } from 'react';
...
function buildDisplaySources(sources) {
  ...
  return [...map.values()].map((row) => {
    ...
    const label = t && n ? `${t} (${n})` : t || n || 'Văn bản pháp luật';
    return { ...row, excerpt, label };
  });
}
...
function renderMarkdownBold(text) {
  ...
  for (let i = 0; i < parts.length; i++) {
    if (i % 2 === 0) {
      ...
      nodes.push(
        <strong key={`b-${i}`} className={styles.mdStrong}>
          {parts[i]}
        </strong>
      );
    }
...
function renderMarkdownBlocks(text) {
  ...
  for (const line of lines) {
    ...
    if (m) {
      flushPara();
      const level = Math.min(Math.max(m[2].length, 1), 6);
      const titleText = m[3];
      const tag = `h${level}`;
      const cls = MD_HEADING_CLASS[level - 1] || styles.mdH3;
      out.push(createElement(tag, { key: `h-${k++}`, className: cls }, renderMarkdownBold(titleText)));
    } else {
      para.push(line);
    }
  }
  flushPara();
  return out.length > 0 ? out : renderMarkdownBold(s);
}
...
export default function ChatAiMessage({ content, metadata }) {
  ...
  const displayText = useMemo(() => {
    const raw = stripBoilerplateLead(cleanDisplayText(content || ''));
  ...
  const openSource = useCallback(async (row) => {
    if (row.documentId) {
      setPanel({ phase: 'loading', label: row.label, documentId: row.documentId });
      try {
        const doc = await documentAPI.getDetail(row.documentId);
        setPanel({
          phase: 'full',
          label: row.label,
          documentId: row.documentId,
          title: doc?.title || row.label,
          documentNumber: doc?.documentNumber ?? row.documentNumber,
          documentType: doc?.documentType ?? row.documentType,
  ...
  return (
    <>
      <div className={styles.wrap}>
        <div className={styles.body}>{renderMarkdownBlocks(displayText)}</div>
          ...
          >
            ...
            {panel.phase === 'full' && panel.documentId ? (
              <div className={styles.panelFooter}>
                <Link href="/dashboard/documents" className={styles.docLink}>
                  Mở trang Tra cứu văn bản →
                </Link>
              </div>
            ) : null}
          </div>
